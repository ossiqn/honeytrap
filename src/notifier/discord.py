...
class DiscordNotifier:
    ...
    def send_attack(self, attack: Dict) -> bool:
        ...
        try:
            ...
            embed = DiscordEmbed(
                ...
                description=f"**{attack.get('attack_type', 'Unknown').replace('_', ' ').upper()}**",
            ...
            embed.add_embed_field(name="🖥️ Attacker IP",  value=f"`{attack.get('attacker_ip', 'Unknown')}`",    inline=True)
            ...
            embed.add_embed_field(name="🔥 Threat Score", value=f"`{attack.get('threat_score', 0)}/100`",       inline=True)
            embed.add_embed_field(name="🪪 VPN/Proxy",    value=f"`{'YES' if attack.get('is_vpn') else 'NO'}`", inline=True)
            ...
            if attack.get("payload") and len(attack.get("payload", "")) > 0:
                payload_preview = attack.get("payload", "")[:200]
                embed.add_embed_field(name="📦 Payload", value=f"```{payload_preview}```", inline=False)

            embed.set_footer(text="HoneyTrap Network — Produced by ossiqn | ossiqn.com.tr")
            embed.set_timestamp()
    ...
    def send_summary(self, stats: Dict):
        ...
        try:
            ...
            embed = DiscordEmbed(
                title="📊 HoneyTrap Network — Daily Summary",
                description=f"Total attacks captured: **{stats.get('total', 0)}**",
                color=0x00AAFF
            )
            ...
            embed.add_embed_field(name="🧬 IOC Count",     value=str(stats.get("ioc_count", 0)),     inline=True)
            ...
            top_countries = stats.get("country_counts", {})
            if top_countries:
                country_str = "\n".join([f"`{k}`: {v}" for k, v in list(top_countries.items())[:5]])
                embed.add_embed_field(name="🌍 Top Countries", value=country_str, inline=False)
