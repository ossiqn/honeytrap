...
class IOCManager:
    ...
    def process_attack(self, attack: Dict):
        ...
        if ip:
            ...
            if self.attack_counts[ip] >= threshold:
                auto_blacklist = self.config.get("detection", {}).get("auto_blacklist", True)
                if auto_blacklist:
                    self.db.add_to_blacklist(ip, "auto_blacklist_threshold_exceeded")
                    logger.warning(f"[ossiqn] Auto-blacklisted IP: {ip} (hit count: {self.attack_counts[ip]})")
    ...
    def export_ioc(self) -> Dict:
        ...
        for ioc in iocs:
            ...
            if ioc_type not in result["ioc_types"]:
                result["ioc_types"][ioc_type] = 0
            result["ioc_types"][ioc_type] += 1
